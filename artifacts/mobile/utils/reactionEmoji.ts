...
/** Native emoji keyboards append to the field; keep only the latest sequence. */
export function latestReactionEmoji(value: string): string | null {
  const matches = value.match(new RegExp(EMOJI.source.slice(1, -1), "gu"));
  const latest = matches?.[matches.length - 1];
  return latest && isReactionEmoji(latest) ? latest : null;
}
