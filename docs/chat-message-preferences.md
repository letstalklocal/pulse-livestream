...
### Individual DM video duration validation

Follow-up to the TestFlight pack-video failure: user reported “Invalid media DM” for an individual video. Code inspection confirmed `/dms/media` rejected the fractional millisecond duration returned by the iOS picker. The endpoint now rounds finite nonnegative durations within PostgreSQL's integer range before validation, storage and idempotency comparison. Null/omitted duration remains absent; invalid/negative/overflow values are rejected. MIME, paid price, recipient, chat activation and retry-conflict checks remain intact.

Real-database integration checks pass for fractional duration, rounded storage, same-key raw/rounded retries yielding one DM, changed-duration conflict, paid price preservation, invalid metadata/MIME rejection, photos and authentication/chat-access enforcement. Temporary accounts/messages/follows were removed. Existing client playback and upload-retry tests, API types/build and diff checks pass. Development API was rebuilt/restarted preserving environment; the running DM media endpoint requires authentication. Production publication and Android/iPhone confirmation remain pending. This backend fix can reach the existing TestFlight build via API publication; RevenueCat and mobile dependencies were not changed.
