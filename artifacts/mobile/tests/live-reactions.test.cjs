...
let selectedEmoji = '❤️';
const render = () => { cursor = 0; return api.LiveReactions({ channelId: 'demo', canSend: true, selectedEmoji }); };
...
for (let i=0; i<3; i++) button(tree, 'Send reaction 🔥').props.onPress();
...
socketOptions.onMessage({ data: JSON.stringify({ type: 'reactions_ready' }) }); tree = render(); button(tree, 'Send reaction 🔥').props.onPress();
