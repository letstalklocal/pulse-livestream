...
(async()=>{
 ...
 for (const [type, mimeType, expected] of [['video', undefined, 'video/mp4'], ['video', 'video/quicktime', 'video/quicktime'], ['image', undefined, 'image/jpeg']]) {
  const seen = {};
  const scope = { asset: {type, mimeType, uri:'local', width:100, height:100}, peerId:'2', setUploading(){},setError(){},setAsset(){},onMediaSent(){},onClose(){},File:class{},requestUpload:{mutateAsync:async request=>{seen.request=request.data.contentType;return {uploadUrl:'upload',objectPath:'/objects/fixture'};}},fetch:async(url,request)=>{seen.put=request.headers['Content-Type'];return {ok:true};},sendMediaDm:{mutateAsync:async request=>{seen.message=request.data.contentType;}} };
