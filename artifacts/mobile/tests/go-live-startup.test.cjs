...
async function run(failure) {
  ...
  const scope = {
    ...
    isNative: true, cameraReady: true, engineRef: { current: {} }, isPrivateInvite: false, isPremium: false,
    requiredGiftId: null, invitationChannelId: null, privateInvitationId: null,
...
(async () => { await run('create'); await run('token'); await run(null); console.log('PASS: visible startup failures, failed cleanup resets spinner, successful startup joins intended channel.'); })().catch(e => { console.error(e); process.exitCode = 1; });
