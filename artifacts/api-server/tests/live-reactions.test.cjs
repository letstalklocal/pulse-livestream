...
let now = 100000, blocked = false, party = false, videoEnabled = true;
...
vm.runInNewContext(code, { exports: api, Date: class extends Date { static now() { return now; } }, process: { env: {} }, require(name) {
  ...
  if (name === './creatorVideoAccess') return { availableCreatorVideo: async (id, uid) => id === videoId && !blocked && (videoEnabled || uid === 1) ? { id } : null };
...
(async () => {
  ...
  const videoHost = socket(), videoViewer = socket(), videoAnonymous = socket(), videoOther = socket();
  const videoChannel = `creator-video:${videoId}`;
  await videoAnonymous.subscribe(videoChannel); assert.equal(videoAnonymous.sent.at(-1).type, 'subscription_denied');
  await videoOther.subscribe('creator-video:22222222-2222-4222-8222-222222222222', 'viewer'); assert.equal(videoOther.sent.at(-1).type, 'subscription_denied');
  await videoHost.subscribe(videoChannel, 'host'); await videoViewer.subscribe(videoChannel, 'viewer');
  assert.equal(videoViewer.sent.at(-1).type, 'reactions_ready');
  await videoViewer.message({ type: 'reaction', emoji: '🔥', count: 1 }); assert.equal(videoHost.sent.length, 1, 'videos accept only hearts');
  now += 220; await videoViewer.message({ type: 'reaction', emoji: '❤️', count: 3 });
  assert.equal(videoHost.sent.at(-1).count, 3); assert.equal(videoViewer.sent.length, 1, 'video heart no echo');
  videoEnabled = false; now += 220;
  await videoViewer.message({ type: 'reaction', emoji: '❤️', count: 1 }); assert.equal(videoViewer.ws.readyState, 3, 'turn off video revokes audience reaction access');
  const ownerPreview = socket(); await ownerPreview.subscribe(videoChannel, 'host'); assert.equal(ownerPreview.sent.at(-1).type, 'reactions_ready');
  blocked = true; now += 220; await ownerPreview.message({ type: 'reaction', emoji: '❤️', count: 1 }); assert.equal(ownerPreview.ws.readyState, 3);
