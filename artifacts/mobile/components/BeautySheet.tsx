...
export function BeautySheet({ settings, onChange, error, onClose }: {
  ...
  return <Modal visible transparent animationType="slide" statusBarTranslucent onRequestClose={onClose}>
    <View style={{ flex: 1, justifyContent: "flex-end" }}>
      <Pressable style={{ flex: 1 }} onPress={onClose} accessibilityLabel="Close beauty effects" />
      <View style={{ backgroundColor: "#17171D", borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 24, paddingBottom: insets.bottom + 24 }}>
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
          <Text style={{ color: "#FFF", fontSize: 22, fontWeight: "700" }}>Beauty</Text>
          <TouchableOpacity onPress={onClose} accessibilityLabel="Close" hitSlop={12}><Ionicons name="close" size={24} color="#FFF" /></TouchableOpacity>
        </View>
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginVertical: 20 }}>
          <Text style={{ color: "#FFF", fontSize: 16 }}>Beauty effects</Text>
          <Switch value={settings.enabled} onValueChange={enabled => onChange({ ...settings, enabled })} trackColor={{ false: "#444", true: "#FF1966" }} accessibilityLabel="Beauty effects" />
        </View>
        <View style={{ opacity: settings.enabled ? 1 : 0.4 }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
            <Text style={{ color: "#FFF", fontSize: 15 }}>Smoothness</Text>
          ...
          <View
            ...
            accessible accessibilityRole="adjustable" accessibilityLabel="Smoothness"
        ...
        <Text style={{ color: error ? "#FF819A" : "#999", fontSize: 13 }}>{error ?? "Adjust your look in the preview. Your viewers will see the same effect."}</Text>
      </View>
    </View>
  </Modal>;
}
