...
export const identityVerificationsTable = pgTable("identity_verifications", {
  ...
  verificationType: text("verification_type", { enum: ["selfie", "id"] }),
  idFallbackRequired: boolean("id_fallback_required").notNull().default(false),
  selfieSessionId: text("selfie_session_id"),
  upgradeStatus: text("upgrade_status").notNull().default("not_started"),
  upgradeSessionId: text("upgrade_session_id").unique(),
  upgradeSessionUrl: text("upgrade_session_url"),
  upgradeWorkflowId: text("upgrade_workflow_id"),
  upgradeConsentAt: timestamp("upgrade_consent_at", { withTimezone: true }),
  upgradeConsentVersion: text("upgrade_consent_version"),
  upgradeCheckedAt: timestamp("upgrade_checked_at", { withTimezone: true }),
  upgradeVerifiedAt: timestamp("upgrade_verified_at", { withTimezone: true }),
