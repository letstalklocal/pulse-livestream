...
assert.ok(cards.every(node => node.props.style[0].width === '25%'), 'Four equal columns');
...
assert.ok(cards.every(n => n.props.style[0].height === undefined && n.props.style[0].paddingBottom === 0), 'Gift border wraps content through the bottom of Send');
...
assert.ok(selection.nodes.find(n => n.props.key === 'rose').props.style.some(s => s?.borderColor === '#FF4D85'), 'Selection has a visible border');
...
assert.ok(preview.nodes.some(n => n.type === "View" && Array.isArray(n.props.style) && n.props.style[0]?.maxHeight === "40%"), "Gift drawer is capped at 40 percent of the screen");
