...
**Scope decision (2026-09-20):** defer coin-funded VIP self-purchases and gifting, including profile Gift VIP and stacking. Keep prices/design for later; do not count this deferred feature as a current launch blocker. Admin VIP grants and the streamer's VIP-buyer list remain deferred. Next IAP validation is real Apple sandbox purchase/access, restore, renewal/expiry and refund testing for the direct-store products.

**Sandbox lifecycle/logging update (2026-09-20):** RevenueCat event history confirms two monthly Test Store cycles, four renewals each, and expiration. Pulse's final inactive flag/expiry matches the latest provider expiry; user reported VIP disappearing. Historical delivery-by-delivery HTTP/server transitions are unavailable. Persistent authenticated webhook attempt logs now record outcomes and before/after VIP changes; public dev TEST returned 200 and persisted successfully. See [evidence and limitations](revenuecat-integration.md#persistent-webhook-logs-and-verified-sandbox-history--september-20-2026). Do not repeat the earlier claim that no Test Store renewal/expiry evidence exists; Apple StoreKit tests remain pending.

**VIP benefit implemented (2026-09-19):** active VIP viewers can open other users' followers/following lists. Owners keep free access. Reads use Pulse's stored flag/expiry; webhooks and reconciliation maintain access, with sandbox/production isolation. Migration applied in development, automated route/provider/HTTP/database tests passed; deployment migration/configuration and actual phone purchase-to-unlock/lifecycle tests remain required. See [VIP implementation](pulse-vip.md#first-vip-benefit-and-stored-access--september-19-2026).

- [x] Apple launch-country availability for the app, three VIP products and nine coin products: user relayed saved/reopened verification for UK, Saudi Arabia, USA, Colombia, Russia, Spain, Canada, Australia, Venezuela, Mexico, Costa Rica, Argentina and Brazil. All 13 accepted, including Russia. Nothing submitted/released. This supersedes earlier pending availability notes; it does not establish payment or verification-provider support. See [availability checkpoint](revenuecat-integration.md#apple-availability-completed--september-19-2026).

**Latest review follow-up (2026-09-19):** Other session reports preserved VIP/default-package/entitlement mappings and no new products found by Apple import, but RevenueCat still shows Missing Metadata. Our previous explicitly expanded API readback also returned null indicative prices; cause unresolved. No native sandbox purchase has been tested. Missing: 12 genuine IAP review screenshots (3 VIP + 9 coin), reviewer credentials, reviewer contact name/phone/email, purchase-access instructions, and App Store screenshots (inspected section reported 0/10). Availability was subsequently confirmed for all 13 countries; see the completed item above. See [full follow-up](revenuecat-integration.md#revenuecat-metadata-and-review-follow-up--september-19-2026).

**VIP update (2026-09-19):** User relayed saved/reopened Apple products `monthly` (6813990002, $9.99/month), `yearly` (6813990251, $79.99/year), and `lifetime` (6813990639, $99.99 once). Monthly/yearly share Pulse VIP group 22398059, both level 1; lifetime is a non-consumable. All are reported Prepare for Submission. RevenueCat package/entitlement mappings are configured, but independent readback still shows null Apple duration/price metadata. Genuine review screenshots, VIP benefits, synchronization and native sandbox tests remain pending; country availability was subsequently completed. See [full handoff and evidence](revenuecat-integration.md#apple-vip-saved-product-handoff--september-19-2026). No review/release submission occurred.
...
**Development milestone implemented:** staff login, live user directory, and read-only account details. Live overview metrics and the seven-date UTC growth chart are also implemented. Read-only account-removal history and the verification manual-review queue are now implemented (September 20); current-live-stream and moderation-report lists are also implemented; wallet and audit browsing plus moderation actions remain pending. See the [live-data implementation plan](admin-website.md#live-data-implementation-plan--september-13-2026) for data sources, sequencing, permissions, and acceptance checks. The initial admin is `one.espana@gmail.com`; bind membership to its verified Clerk identity. Admin permissions must remain separate from ordinary mobile-app access and behavior.
