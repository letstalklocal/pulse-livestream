...
export function PartySheet({ channelId, party, uid, onAction, onClose }: {
  ...
  const button = (label: string, icon: React.ComponentProps<typeof Ionicons>["name"], action: PartyAction["action"], disabled = false) => (
    <TouchableOpacity style={[styles.action, disabled && { opacity: 0.4 }]} disabled={busy || disabled} onPress={() => void act(action)} accessibilityRole="button">
      <Ionicons name={icon} color="#FFF" size={21} /><Text style={styles.actionText}>{label}</Text>
    </TouchableOpacity>
  );
  return (
    <Modal visible transparent animationType="slide" onRequestClose={onClose} statusBarTranslucent>
      <View style={styles.backdrop}>
        <TouchableOpacity style={StyleSheet.absoluteFill} onPress={onClose} accessibilityLabel="Close Party" />
        <View style={[styles.sheet, { paddingBottom: insets.bottom + 20 }]}>
          <View style={styles.heading}><Text style={styles.title}>Party</Text><TouchableOpacity onPress={onClose} accessibilityLabel="Close Party" style={styles.close}><Ionicons name="close" size={24} color="#FFF" /></TouchableOpacity></View>
          {party && peer ? <>
            ...
            {party.status === "pending" ? <>
              <Text style={styles.status}>{incoming ? "Invited you to Party" : "Waiting for their response"}</Text>
              ...
              <Text style={styles.status}>{party.ready ? "Party connected" : "Connecting both hosts..."}</Text>
            ...
            {candidates.isLoading ? <ActivityIndicator color="#FF1966" /> : candidates.isError ? <TouchableOpacity onPress={() => void candidates.refetch()}><Text style={styles.status}>{candidateError}</Text></TouchableOpacity> : candidates.data?.users.length ? candidates.data.users.map(person => (
              <TouchableOpacity key={person.channelId} disabled={busy} style={styles.person} onPress={() => void act("invite", person.channelId)} accessibilityLabel={`Invite ${person.name} to Party`}>
            ...
            )) : <Text style={styles.status}>No live hosts available right now</Text>}
          </ScrollView>}
