...
### Persistent upload feedback — September 21, 2026

User reported returning from video selection/upload to an ordinary-looking preview with no visible explanation until the video appeared roughly 30 seconds later. The existing sheet placed upload progress and processing rows below the main preview. Keep a status panel above the scrolling content on both Video and History (including stats), beginning when the picker opens and continuing through transfer and provider processing. Show actual upload percentage; after transfer reaches 100%, show Processing video, then actual encoding percentage/Finalizing from existing polling. Remove the panel when no transfer or pending video remains. Do not invent an ETA or treat transfer completion as playback readiness. Existing per-video failure/removal, cancellation, selection, history and Discovery opt-in behavior remain.

This creator-video path already uses Bunny delivery and resumable TUS chunks with in-session offset recovery. DM/pack object-storage transfers are separate; this feedback change does not migrate them, enable cross-restart resumption, or add a CDN for private media. Consistent upload feedback/recovery across the app remains the intended broader direction.

Automated focused checks cover pinned feedback, upload-to-processing labels, visibility across tabs, existing polling/removal races, history/selection and interrupted chunk recovery. Required stream regression suite passes. Android/iPhone picker return, processing wait, failure/cancel, background/return and preview appearance require device confirmation; the reported delay's exact on-device cause remains unverified.
