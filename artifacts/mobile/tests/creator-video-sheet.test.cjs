...
(async () => {
  ...
  state[5] = 42;
  assert.ok(render().some(n => n.type === 'Text' && n.props.children.includes('Uploading: 42%')));
  state[5] = 100;
  assert.ok(render().some(n => n.type === 'Text' && n.props.children.includes('Processing video…')), 'completed transfer is not presented as video readiness');
  state[5] = null;
  state[7] = 'history';
  assert.ok(render().some(n => n.props?.testID === 'creator-video-upload-status'), 'processing feedback persists on History');
  state[7] = 'video';
