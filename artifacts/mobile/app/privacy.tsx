...
export default function PrivacyScreen() {
  ...
  return <View style={{ flex: 1, backgroundColor: colors.background }}>
    <View style={[styles.header, { paddingTop: (Platform.OS === "web" ? 67 : insets.top) + 10, borderColor: colors.border }]}><TouchableOpacity accessibilityLabel={t("Back")} onPress={() => showBlocked ? setShowBlocked(false) : router.back()} style={[styles.back, { backgroundColor: colors.card, borderColor: colors.border }]}><Ionicons name="chevron-back" size={20} color={colors.foreground} /></TouchableOpacity><Text style={[localizedTextStyle(), [styles.title, { color: colors.foreground }]]}>{showBlocked ? t("Blocked accounts") : t("Privacy")}</Text><View style={{ width: 38 }} /></View>
    <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: insets.bottom + 32 }}>
      {!showBlocked ? <>
        {privacy.isPending && <ActivityIndicator color={colors.primary} />}
        {privacy.isError && <TouchableOpacity accessibilityRole="button" onPress={() => { void privacy.refetch(); }}><Text style={[localizedTextStyle(), styles.error]}>{t("Couldn’t load privacy settings. Tap to retry.")}</Text></TouchableOpacity>}
        {privacy.save.isError && <Text accessibilityRole="alert" style={styles.error}>{t(privacy.save.error.message)}</Text>}
        ...
        <Text style={[localizedTextStyle(), [styles.heading, { color: colors.mutedForeground }]]}>{t("POSTS")}</Text>
        <TouchableOpacity accessibilityRole="button" disabled={!privacy.isSuccess || privacy.save.isPending} style={[styles.menuRow, { backgroundColor: colors.card, borderColor: colors.border }]} onPress={() => setChoice("postsVisibility")}><Text style={[localizedTextStyle(), [styles.label, { color: colors.foreground, flex: 1 }]]}>{t("Who can see Posts")}</Text><Text style={[localizedTextStyle(), { color: colors.mutedForeground }]}>{privacy.preferences.postsVisibility === "friends" ? t("Friends") : t("Everyone")}</Text><Ionicons name="chevron-forward" size={17} color={colors.mutedForeground} /></TouchableOpacity>
        <Text style={[localizedTextStyle(), [styles.detail, { color: colors.mutedForeground, marginBottom: 12 }]]}>{t("Friends are people you follow who follow you back.")}</Text>
        <Text style={[localizedTextStyle(), [styles.heading, { color: colors.mutedForeground }]]}>{t("BLOCKED ACCOUNTS")}</Text>
        <TouchableOpacity accessibilityRole="button" style={[styles.menuRow, { backgroundColor: colors.card, borderColor: colors.border }]} onPress={() => { setShowBlocked(true); void list.refetch(); }}><Ionicons name="person-remove-outline" size={22} color={colors.primary} /><View style={{ flex: 1, gap: 5 }}><Text style={[localizedTextStyle(), [styles.label, { color: colors.foreground }]]}>{t("Blocked accounts")}</Text><Text style={[localizedTextStyle(), [styles.detail, { color: colors.mutedForeground }]]}>{t("Blocks apply across the app")}</Text></View>{total !== undefined && <Text style={{ color: colors.mutedForeground }}>{total}</Text>}<Ionicons name="chevron-forward" size={17} color={colors.mutedForeground} /></TouchableOpacity>
        <Text style={[localizedTextStyle(), [styles.description, { color: colors.mutedForeground, marginTop: 14 }]]}>{t("Blocking stops contact, gifts, invitations, and access to each other’s profiles, posts, and live streams while signed in. You can block someone from their profile, chat, or your live viewer controls.")}</Text>
      </> : <>
        <Text style={[localizedTextStyle(), [styles.description, { color: colors.mutedForeground }]]}>{t("These accounts are blocked across Pulse. Unblocking here clears the block everywhere. Existing messages are kept for reporting.")}</Text>
    ...
    <Modal visible={!!choice} transparent animationType="fade" onRequestClose={() => { if (!privacy.save.isPending) setChoice(null); }}>
      <View style={styles.scrim}><View style={[styles.modal, { backgroundColor: colors.card }]}><Text style={[localizedTextStyle(), [styles.title, { color: colors.foreground }]]}>{choice === "partyInvites" ? t("Who can invite Party") : t("Who can see Posts")}</Text><Text style={[localizedTextStyle(), [styles.description, { color: colors.mutedForeground, marginTop: 14 }]]}>{t("Friends are people you follow who follow you back. Blocked accounts remain blocked.")}</Text>
        {privacy.save.isError && <Text accessibilityRole="alert" style={styles.error}>{t(privacy.save.error.message)}</Text>}
        {(["everyone", "friends"] as const).map(value => <TouchableOpacity key={value} accessibilityRole="radio" accessibilityState={{ selected: !!choice && privacy.preferences[choice] === value }} disabled={privacy.save.isPending} style={[styles.choice, { borderColor: colors.border }]} onPress={() => { if (choice) privacy.save.mutate({ [choice]: value }, { onSuccess: () => setChoice(null) }); }}><Text style={[localizedTextStyle(), [styles.label, { color: colors.foreground }]]}>{value === "everyone" ? t("Everyone") : t("Friends")}</Text>{choice && privacy.preferences[choice] === value && <Ionicons name="checkmark" size={21} color={colors.primary} />}</TouchableOpacity>)}
        <TouchableOpacity accessibilityRole="button" disabled={privacy.save.isPending} onPress={() => setChoice(null)} style={{ padding: 18, alignItems: "center" }}><Text style={[localizedTextStyle(), { color: colors.foreground }]}>{t("Cancel")}</Text></TouchableOpacity>
      </View></View>
    </Modal>
    <Modal visible={!!selected} transparent animationType="fade" onRequestClose={() => { if (!unblock.isPending) setSelected(null); }}>
      <View style={styles.scrim}><View style={[styles.modal, { backgroundColor: colors.card }]}><Text style={[localizedTextStyle(), [styles.title, { color: colors.foreground }]]}>{t("Unblock {v0}?", { v0: selected?.name })}</Text><Text style={[localizedTextStyle(), [styles.description, { color: colors.mutedForeground, marginTop: 14 }]]}>{t("This removes your block across Pulse. You can interact again unless they have also blocked you. Temporary live-room restrictions still apply.")}</Text>{unblock.isError && <Text accessibilityRole="alert" style={styles.error}>{t(unblock.error.message)}</Text>}<TouchableOpacity accessibilityRole="button" disabled={unblock.isPending} onPress={() => { if (selected) unblock.mutate(selected); }} style={[styles.button, { backgroundColor: colors.primary }]}>{unblock.isPending ? <ActivityIndicator color="#FFF" /> : <Text style={[localizedTextStyle(), { color: "#FFF", fontWeight: "600", fontSize: 15 }]}>{t("Unblock")}</Text>}</TouchableOpacity><TouchableOpacity accessibilityRole="button" disabled={unblock.isPending} onPress={() => setSelected(null)} style={{ padding: 18, alignItems: "center" }}><Text style={[localizedTextStyle(), { color: colors.foreground }]}>{t("Cancel")}</Text></TouchableOpacity></View></View>
    </Modal>
  </View>;
}
