...
export default function MediaPacksScreen() {
  ...
  const choose = async () => {
    if (nativeOnly()) { Alert.alert(t("Native app required"), t("Selecting and uploading media packs is available in the iOS or Android app.")); return; }
    const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (!permission.granted) { Alert.alert(t("Permission needed"), t("Allow photo library access to add media. You can enable it in Settings.")); return; }
  ...
  return <View style={[styles.page, { backgroundColor: colors.background, paddingTop: insets.top + 8 }]}>
    <View style={styles.header}><TouchableOpacity onPress={() => router.back()}><Ionicons name="chevron-back" size={25} color={colors.foreground}/></TouchableOpacity><Text style={[localizedTextStyle(), [styles.title,{color:colors.foreground}]]}>{t("Media Packs")}</Text><TouchableOpacity onPress={() => setVisible(true)} testID="pack-create-open"><Ionicons name="add-circle" size={28} color={colors.primary}/></TouchableOpacity></View>
    <Text style={[localizedTextStyle(), [styles.subtitle,{color:colors.mutedForeground}]]}>{t("Sell a reusable set of photos and videos in DMs.")}</Text>
    <FlatList data={packs} keyExtractor={(p) => p.id} contentContainerStyle={styles.list} renderItem={({item}) => <View style={[styles.pack,{backgroundColor:colors.card,borderColor:colors.border}]}><View style={[styles.packIcon,{backgroundColor:"rgba(255,25,102,.14)"}]}><Ionicons name="images" size={21} color={colors.primary}/></View><View style={{flex:1}}><Text style={[styles.packName,{color:colors.foreground}]}>{item.name}</Text><Text style={[localizedTextStyle(), [styles.meta,{color:colors.mutedForeground}]]}>{t("{v0} items · 🪙 {v1}", { v0: item.itemCount, v1: item.price })}</Text></View><TouchableOpacity onPress={() => remove.mutate({ packId: item.id } as any)} testID={`pack-delete-${item.id}`}><Ionicons name="trash-outline" size={19} color={colors.mutedForeground}/></TouchableOpacity></View>} ListEmptyComponent={<View style={styles.empty}><Ionicons name="albums-outline" size={48} color={colors.mutedForeground}/><Text style={[localizedTextStyle(), [styles.emptyTitle,{color:colors.foreground}]]}>{t("Your vault is empty")}</Text><Text style={[localizedTextStyle(), [styles.emptyCopy,{color:colors.mutedForeground}]]}>{t("Create a pack once, then share it with anyone.")}</Text><TouchableOpacity onPress={() => setVisible(true)} style={[styles.create,{backgroundColor:colors.primary}]} testID="pack-create-empty"><Text style={[localizedTextStyle(), styles.createText]}>{t("Create a pack")}</Text></TouchableOpacity></View>}/>
    <Modal visible={visible} animationType="slide" onRequestClose={() => setVisible(false)}><View style={[styles.modal,{backgroundColor:colors.background,paddingTop:insets.top+12}]}><View style={styles.header}><Text style={[localizedTextStyle(), [styles.title,{color:colors.foreground}]]}>{t("New pack")}</Text><TouchableOpacity onPress={() => setVisible(false)}><Ionicons name="close" size={26} color={colors.foreground}/></TouchableOpacity></View><ScrollView contentContainerStyle={styles.form}><TextInput value={name} onChangeText={setName} placeholder={t("Pack name")} placeholderTextColor={colors.mutedForeground} style={[styles.input,{color:colors.foreground,borderColor:colors.border,backgroundColor:colors.card}]}/><TextInput value={price} onChangeText={setPrice} placeholder={t("Coin price")} keyboardType="number-pad" placeholderTextColor={colors.mutedForeground} style={[styles.input,{color:colors.foreground,borderColor:colors.border,backgroundColor:colors.card}]}/><TouchableOpacity onPress={choose} style={[styles.addMedia,{borderColor:colors.primary}]} testID="pack-media-picker"><Ionicons name="add" size={23} color={colors.primary}/><Text style={[localizedTextStyle(), [styles.addMediaText,{color:colors.primary}]]}>{t("Add photos or videos · {v0}/20", { v0: assets.length })}</Text></TouchableOpacity><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.thumbs}>{assets.map((asset,index)=><View key={`${asset.uri}-${index}`} style={styles.thumb}><Image source={{uri:asset.uri}} style={styles.image}/>{asset.type==="video"&&<Ionicons name="videocam" size={16} color="#FFF" style={styles.video}/>}<TouchableOpacity onPress={()=>setAssets((a)=>a.filter((_,i)=>i!==index))} style={styles.remove} testID={`pack-media-remove-${index}`}><Ionicons name="close" size={13} color="#FFF"/></TouchableOpacity></View>)}</ScrollView>{error&&<Text style={styles.error}>{t(error)}</Text>}<TouchableOpacity onPress={save} disabled={saving} style={[styles.create,{backgroundColor:saving?colors.muted:colors.primary}]} testID="pack-create-submit"><Text style={[localizedTextStyle(), styles.createText]}>{saving ? t("Uploading {v0} item{v1}…", { v0: assets.length, v1: assets.length===1?"":"s" }) : t("Create media pack")}</Text></TouchableOpacity></ScrollView></View></Modal>
  </View>;
}
